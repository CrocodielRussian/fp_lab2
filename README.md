...
- Тестирование (unit) [test/Spec.hs](test/Spec.hs)
...
Основные операции:
- `insert` / `delete` — добавление и удаление с автоматической балансировкой.
- `filterOA` / `mapOA` — функциональные преобразования.
- `foldlOA` / `foldrOA` — свертки.
- `member` — проверка принадлежности за счёт адресации по хешу.
...
# Итог
Создана неизменяемая полиморфная структура данных, с операциями добавления/удаления, фильтрации, отображения и сверток. Реализованы эффективное сравнение без преобразования к спискам и подробное тестирование: HUnit проверяет базовые сценарии.
